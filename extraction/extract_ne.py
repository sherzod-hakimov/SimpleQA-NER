...
def extract_subjects(sentence, dict, max_ngram_size, stopwords, exclude_small_ngrams, exclude_stop_words, stemmer, subject_triple_counts):
    candidates = list()
        ...
        for ngram_tokens in extracted_ngrams:
            ...
            if normalized_ngram in dict.keys():
                ...
                for uri in matches.keys():
                    ##don't add uris that don't any triples
                    if uri in subject_triple_counts.keys():
                        if subject_triple_counts[uri] > 8:
                            freq = matches[uri]
    ...
    for d in dataset_names:
        ...
        with open(dataset_path, encoding="utf-8") as f:
            ...
            for i, line in enumerate(content):
                ...
                if target_subject in subject_triple_counts.keys():
                    avg_triple_count += subject_triple_counts[target_subject]
                    min_triple_count = min(min_triple_count, subject_triple_counts[target_subject])
                    max_triple_count = max(max_triple_count, subject_triple_counts[target_subject])

                candidates = extract_subjects(text, mention_dict, max_ngram_size, stopwords, exclude_small_ngrams,
                                            exclude_stop_words, stemmer, subject_triple_counts)

                subjects = list()

                for start_index, end_index, ngram, uri, freq in candidates:
                    triple_count = subject_triple_counts[uri]
                    subjects.append([start_index, end_index, ngram, uri, freq, triple_count])


                number_of_candidates += len(subjects)
                subjects.sort(key=lambda tup: tup[5])  # sorts in place
                ...
                for range in recall_ranges:
                    ...
                    for start_index, end_index, ngram, uri, freq, triple_count in filtered_subjects:
            ...
            print("Average triple count: " + str(avg_triple_count/float(total_count)))
            print("Max triple count: " + str(max_triple_count))
            print("Min triple count: " + str(min_triple_count))
